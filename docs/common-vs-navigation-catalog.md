...
| `repoUi` | Nhan UI noi bo cua repo (VD: Quick Access, Open Main Section) | Nhan duoc xem la PCRM source-like navigation |
| `site.contact` | Dia chi/email/phone dung lap lai o footer va cac khoi contact | Hardcode truc tiep trong JSX |
...
6. Nhan repo-only UI phai dat trong `repoUi.*` de khong lan voi PCRM navigation labels.
7. Footer contact/legal repeated text phai doc tu locale (`site.contact.*`, `footerLegal.*`).
...
5. Header/Footer khong con fallback hardcode EN/VI cho nhan UI lap lai.
