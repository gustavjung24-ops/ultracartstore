...
export default function AuthorAboutBox({ author }: AuthorAboutBoxProps) {
  ...
  return (
    <section className="mt-10 rounded-2xl border border-[#d9e4eb] bg-[#f8fbfd] p-5 font-sans md:p-6">
      <p className="text-sm font-semibold tracking-[0.01em] text-[#0f5c73]">Về tác giả</p>
      <div className="mt-3 flex items-start gap-4">
        ...
        <div className="min-w-0">
          ...
          {degreeLine ? (
            <p className="mt-1 text-sm font-medium tracking-[0.01em] text-slate-500">{degreeLine}</p>
          ) : null}
          ...
          <Link
            ...
            className="mt-4 inline-flex rounded-full border border-[#0f5c73] px-5 py-2 text-sm font-semibold tracking-[0.01em] text-[#0f5c73] no-underline transition hover:bg-[#0f5c73] hover:text-white"
          >
            Xem hồ sơ tác giả
          </Link>
        </div>
